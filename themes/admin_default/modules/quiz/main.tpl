...
<div id="content">
    ...
    <div class="panel panel-default">
		<div class="panel-heading">
			<h3 class="panel-title" style="float:left"><i class="fa fa-pencil"></i> {LANG.quiz_list}</h3>
		...
		<div class="panel-body">
			...
			<form action="" method="post" enctype="multipart/form-data" id="form-rows" class="form-horizontal">
				...
				<table class="table table-striped table-bordered table-hover">
					<thead>
						<tr >
							<td class="text-center" width="10">
 							...
 							<td align="center"><a href="{URL_FULL_NAME}">{LANG.info_full_name}</a></td>
							<td align="center"><a href="{URL_FULL_NAME}">{LANG.info_birthday}</a></td>
							<td align="center"><a href="{URL_TELEPHONE}">{LANG.info_telephone}</a></td>
							<td align="center"><a href="{URL_EMAIL}">{LANG.info_email}</a></td>
					...
					<tbody>
					<!-- BEGIN: loop -->
						<tr class="text-center" id="group_{LOOP.info_id}">
							<td class="text-center">
								<input type="checkbox" name="selected[]" value="{LOOP.info_id}" >
							</td>
		 
							<td align="left">{LOOP.full_name}</td>
							<td align="center">{LOOP.birthday}</td>
							<td align="left">{LOOP.telephone}</td>
							<td align="left">{LOOP.email}</td>
							<td align="center">{LOOP.outcome}</td>
							<td align="center">{LOOP.time}</td>
							<td class="text-center">
 								<a href="{LOOP.link}" data-toggle="tooltip" title="{LANG.view}" class="btn btn-primary"><i class="fa fa-eye"></i></a> 
								<a href="javascript:void(0);" onclick="delete_rows('{LOOP.info_id}', '{LOOP.token}')" data-toggle="tooltip" title="{LANG.delete}" class="btn btn-danger"><i class="fa fa-trash-o"></i>
							</td>
						</tr>
					<!-- END: loop -->
					<tbody>
...
function delete_rows(info_id, token) {
	if( confirm( '{LANG.confirm}' )) 
	{
		$.ajax({
			...
			type: 'post',
			dataType: 'json',
			data: 'info_id=' + info_id + '&token=' + token,
