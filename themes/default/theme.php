...
function nv_site_theme($contents, $full = true)
{
	...
	if (file_exists(NV_ROOTDIR . '/' . NV_ASSETS_DIR . '/htmlheader.txt')) {
            $codeheader = file_get_contents(NV_ROOTDIR . '/' . NV_ASSETS_DIR . '/htmlheader.txt');
            $codeheader=htmlspecialchars($codeheader);
            $codeheader=str_replace('<script','<script data-show="inline"',$codeheader);
            $codeheader=htmlspecialchars_decode($codeheader);
           if(!empty($codeheader)){$my_head .= $codeheader;}
        }
    if (file_exists(NV_ROOTDIR . '/' . NV_ASSETS_DIR . '/htmlfooter.txt')) {
            $codefooter = file_get_contents(NV_ROOTDIR . '/' . NV_ASSETS_DIR . '/htmlfooter.txt');
            if(!empty($codefooter)){$my_footer .= $codefooter;}
        }
