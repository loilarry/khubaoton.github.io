...
if( ! nv_function_exists( 'nv_block_global_sliders' ) )
{
	function nv_block_data_config_sliders( $module, $data_block, $lang_block )
	{
		global $db, $language_array;

		$html = "<select class=\"form-control w400\" name=\"config_idplanbanner\">\n";
	...
	function nv_block_data_config_sliders_submit( $module, $lang_block )
	{
		global $nv_Request;
		$return = array();
		$return['error'] = array();
	...
	{
		global $global_config, $client_info, $my_head;
		...
		if( ! empty( $array_banners_content ) )
		{
			...
			if( file_exists( NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/blocks/global.sliders_copy.tpl' ) )
			{
				$block_theme = $global_config['module_theme'];
			}
			elseif( file_exists( NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/blocks/global.sliders_copy.tpl' ) )
			{
			...
			$xtpl = new XTemplate( 'global.sliders_copy.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks' );
