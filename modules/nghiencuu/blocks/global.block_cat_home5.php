...
if (!nv_function_exists('nv_block_tmscathome5')) {
    /**
     * nv_block_config_tmscathome5()
     *
     * @param string $module
     ...
     * @return string
     */
    function nv_block_config_tmscathome5($module, $data_block, $lang_block)
    {
        ...
        $html .= '<div class="col-sm-18"><input type="text" class="form-control" name="config_d480" size="5" value="' . $data_block['d480'] . '"/></div>';
        $html .= '</div>'; 
        $html .= '<div class="form-group">';
        $html .= '<label class="control-label col-sm-6">Kích thước 640:</label>';
        $html .= '<div class="col-sm-18"><input type="text" class="form-control" name="config_d640" size="5" value="' . $data_block['d640'] . '"/></div>';
        $html .= '</div>'; 
        $html .= '<div class="form-group">';
        $html .= '<label class="control-label col-sm-6">Kích thước 768:</label>';
        $html .= '<div class="col-sm-18"><input type="text" class="form-control" name="config_d768" size="5" value="' . $data_block['d768'] . '"/></div>';
        $html .= '</div>'; 
        $html .= '<div class="form-group">';
        $html .= '<label class="control-label col-sm-6">Kích thước 1024:</label>';
        $html .= '<div class="col-sm-18"><input type="text" class="form-control" name="config_d1024" size="5" value="' . $data_block['d1024'] . '"/></div>';
        $html .= '</div>'; 
        $html .= '<div class="form-group">';
        $html .= '<label class="control-label col-sm-6">Kích thước 1124:</label>';
        $html .= '<div class="col-sm-18"><input type="text" class="form-control" name="config_d1124" size="5" value="' . $data_block['d1124'] . '"/></div>';
        $html .= '</div>'; 
        $html .= '<div class="form-group">';
    ...
    /**
     * nv_block_config_tmscathome5_submit()
     *
     * @param string $module
     * @param array  $lang_block
     * @return array
     */
    function nv_block_config_tmscathome5_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = [];
        $return['error'] = [];
    ...
    /**
     * nv_block_tmscathome5()
     *
     * @param array $block_config
     * @return string|void
     */
    function nv_block_tmscathome5($block_config)
    {
        ...
        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/news/block_tmshome5.tpl')) {
                $block_theme = $global_config['module_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block_tmshome5.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news');
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        ...
        $content = nv_block_tmscathome5($block_config);
    }
}
